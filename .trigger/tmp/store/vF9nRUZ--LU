import {
  metadata,
  task
} from "../../../../../../chunk-I5LANLPO.mjs";
import "../../../../../../chunk-JIQK57WG.mjs";
import {
  __commonJS,
  __esm,
  __export,
  ...
  "node_modules/@supabase/realtime-js/dist/main/lib/serializer.js"(exports) {
    ...
    var Serializer = class {
      ...
      _encodeUserBroadcastPush(message, encodingType, encodedPayload) {
        ...
        const metaLength = this.USER_BROADCAST_PUSH_META_LENGTH + joinRef.length + ref.length + topic.length + userEvent.length + metadata2.length;
...
var NamespaceOperations = class {
  ...
  async createNamespace(id, metadata2) {
    const request = {
  ...
  async createNamespaceIfNotExists(id, metadata2) {
    try {
      return await this.createNamespace(id, metadata2);
    } catch (error) {
...
var IcebergRestCatalog = class {
   ...
   */
  async createNamespace(id, metadata2) {
    return this.namespaceOps.createNamespace(id, metadata2);
  }
  /**
   ...
   */
  async createNamespaceIfNotExists(id, metadata2) {
    return this.namespaceOps.createNamespaceIfNotExists(id, metadata2);
  }
  /**
...
__name(createTaskSupabase, "createTaskSupabase");
...
__name(buildCompanyName, "buildCompanyName");
function buildLeadName(row) {
...
__name(buildLeadName, "buildLeadName");
function updateProgress(progress) {
  metadata.set("progress", progress);
}
__name(updateProgress, "updateProgress");
async function processCSVInBatches(supabase, csvText, ingestionId, totalRows) {
  const records = parse(csvText, {
    columns: true,
    skip_empty_lines: true,
    trim: true
  });
  const companyCache = /* @__PURE__ */ new Map();
  const pendingLeads = [];
  let processedRows = 0;
  let skipped = 0;
  let leadCount = 0;
  ...
  for (let i = 0; i < companyEntries.length; i += COMPANY_BATCH_SIZE) {
    ...
    const { data: companies, error } = await supabase.from("companies").upsert(companyData, { onConflict: "name_normalized" }).select("id, name_normalized");
    if (error) {
...
__name(processCSVInBatches, "processCSVInBatches");
var csvImportTask = task({
  id: "csv-import",
  maxDuration: 600,
  // 10 minutes max
  retry: {
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      await supabase.from("lead_ingestions").update({
        status: "processing",
      ...
      const { data: fileData, error: downloadError } = await supabase.storage.from("csv-uploads").download(storagePath);
...
//# sourceMappingURL=csv-import.mjs.map
